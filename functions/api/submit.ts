...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  if (!env.DB) return json(500, { ok: false, error: "DB binding missing" });
  ...
  await env.DB.prepare(
    `INSERT INTO leads (id, company_name, contact_name, email, phone, employee_range, firewall_vendor, vpn_technology, zero_trust_vendor,
                        consent_contact, consent_tracking, discount_opt_in, created_at, updated_at, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    leadId,
    body.company_name,
