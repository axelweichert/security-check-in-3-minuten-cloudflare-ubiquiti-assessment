...
function riskLabelDe(risk: string) {
  return risk === "low" ? "Niedriges Risiko" : risk === "medium" ? "Mittleres Risiko" : "Hohes Risiko";
}
...
export const onRequestGet: PagesFunction<Env> = async ({ params, env }) => {
  ...
  try {
    ...
    const newPage = () => {
      ...
      curPage.drawText(lang.startsWith("de") ? "Ihre Antworten (Fortsetzung)" : "Your answers (continued)", {
        x: M,
        y: curY,
        size: 12,
        font: fontBold,
