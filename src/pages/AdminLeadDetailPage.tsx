            ...
            <dl className="space-y-4">
                {answers.map(answer => (
                    <div key={answer.id} className="flex flex-col">
                        <dt className="text-sm font-medium text-muted-foreground">{t(allQuestions.find(q => q.id === answer.question_key)?.labelKey || answer.question_key)}</dt>
...
export default function AdminLeadDetailPage() {
  ...
  const groupedAnswers = useMemo(() => {
    ...
    return Object.groupBy(data.answers, (a: LeadAnswer) => getAnswerGroup(a.question_key));
  }, [data?.answers]);
  if (isLoading) {
    return (
