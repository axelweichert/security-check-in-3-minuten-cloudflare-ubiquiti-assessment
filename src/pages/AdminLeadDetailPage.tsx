            ...
            return (
              <div key={answer.id} className="flex flex-col">
                <dt className="text-sm font-medium text-muted-foreground">
                  {t(question?.labelKey ?? answer.question_key)}
                </dt>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="space-y-8">
        ...
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {new Date(lead.created_at).toLocaleDateString(lang, {
                day: '2-digit',
                month: 'long',
                year: 'numeric',
                hour: '2-digit',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
          ...
          {groupedAnswers['awareness'] && (
            <AnswerGroupCard title={t('score.awareness_title')} answers={groupedAnswers['awareness']} t={t} />
          )}
        </div>
        ...
        <div className="space-y-8 lg:sticky lg:top-24">
          <Card className="shadow-md">
            ...
            <CardContent className="space-y-4">
              <InfoItem icon={User} label={t('contact.contact_person')}>
                {lead.contact_name}
              </InfoItem>
          ...
          <Card className="shadow-md">
            <CardHeader>
              <CardTitle>{t('score.stack')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <p>
                <strong>{t('q.firewall_vendor')}:</strong>{' '}
                <Badge variant="secondary">{getDisplayValue('firewall_vendor', lead.firewall_vendor || 'unknown', t)}</Badge>
              </p>
              <p>
                <strong>{t('q.vpn_technology')}:</strong>{' '}
                <Badge variant="secondary">{getDisplayValue('vpn_technology', lead.vpn_technology || 'unknown', t)}</Badge>
              </p>
              <p>
                <strong>{t('q.zero_trust_vendor')}:</strong>{' '}
                <Badge variant="secondary">
                  {getDisplayValue('zero_trust_vendor', lead.zero_trust_vendor || 'unknown', t)}
                </Badge>
              </p>
